...
public class JwtAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
    ...
    */
    ...
    private void handleResponse(HttpServletRequest request, HttpServletResponse response, Authentication authResult, AuthenticationException failed) throws IOException, ServletException {
        // TODO#guang 跳转到 Controller 去处理
        ObjectMapper mapper = new ObjectMapper();
        response.setHeader("Content-Type", "application/json;charset=UTF-8");
        if (authResult != null) {
            // 处理登入成功请求
            PikachuUser user = (PikachuUser) authResult.getPrincipal();
            String token = JwtUtil.sign(user.getUsername(), user.getPassword());
            JsonResult result = JsonResult.ok("Bearer " + token);
            response.setStatus(HttpStatus.OK.value());
            response.getWriter().write(mapper.writeValueAsString(result));
        } else {
            // 处理登入失败请求
            JsonResult<String> result = JsonResult.error("用户名或密码错误");
            result.setErrCode(HttpStatus.BAD_REQUEST.value());
